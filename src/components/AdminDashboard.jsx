// Admin Dashboard
// Shows stats, charts, request management, and announcement posting
...
function AdminDashboard({ adminUser }) {
  ...
  useEffect(() => {
    ...
    unsubs.push(
      onSnapshot(query(collection(db, 'recyclables'), orderBy('submittedAt', 'desc')), (snap) => {
    ...
    unsubs.push(
      onSnapshot(query(collection(db, 'requests'), orderBy('submittedAt', 'desc')), (snap) => {
        setRequests(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
      })
    );

    unsubs.push(
      onSnapshot(query(collection(db, 'announcements'), orderBy('postedAt', 'desc')), (snap) => {
  ...
  const TABS = ['overview', 'requests', 'announcements', 'feedback'];
      ...
      {tab === 'announcements' && (
        <div className="dash-content">
          ...
          <div className="form-card" style={{ marginBottom: '2rem' }}>
            ...
            {annMessage && (
              <div className={`alert alert-${annMessage.type}`}>{annMessage.text}</div>
            )}
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input
                    id="ann-cat"
                    type="text"
                    placeholder="e.g. Event, Reminder, News"
                ...
                <div key={ann.id} className="ann-admin-card">
                  <div className="ann-admin-top">
                    <div>
                      ...
                      {ann.category && (
                        <span className="ann-cat-badge">{ann.category}</span>
                      )}
            ...
            <div className="feedback-list">
              {feedback.map((f) => (
                <div key={f.id} className="feedback-item">
                  ...
                  {f.suggestions && (
                    <p className="fb-suggestions">{f.suggestions}</p>
                  )}
                </div>
              ))}
            </div>
